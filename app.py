...
def create_excel_file_from_db(campaign_number):
    ...
    try:
        ...
        # 🔥 🔥 🔥 🔥 🔥 НАЧАЛО НОВОГО БЛОКА С ТЕКСТОМ РОЛИКА 🔥 🔥 🔥 🔥 🔥
        ws.merge_cells(f"A{row}:B{row}")
        
        # Проверяем есть ли текст ролика
        if user_data.get("campaign_text") and user_data["campaign_text"].strip():
            # Есть текст - показываем его
            ws[f"A{row}"] = "🎙️ ТЕКСТ РОЛИКА:"
            ws[f"A{row}"].font = title_font
            row += 1
            
            # Разбиваем текст на строки и выводим каждую непустую строку
            text_lines = user_data["campaign_text"].split('\n')
            for line in text_lines:
                if line.strip():  # Пропускаем пустые строки
                    ws[f"A{row}"] = line
                    row += 1
        else:
            # Нет текста - значит выбрал "прислать свой ролик" или не ввел текст
            ws[f"A{row}"] = "🎧 ПРОИЗВОДСТВО РОЛИКА:"
            ws[f"A{row}"].font = title_font
            row += 1
            ws[f"A{row}"] = "Клиент предоставит готовый аудиоролик"
            row += 1
        
        ws.append([])
        row += 2
        # 🔥 🔥 🔥 🔥 🔥 КОНЕЦ НОВОГО БЛОКА С ТЕКСТОМ РОЛИКА 🔥 🔥 🔥 🔥 🔥
        ...
        row += 1
        ws[f"A{row}"] = f"• Общий охват за период: ~{formatNumber(total_reach)} чел."
...
def create_campaign():
    ...
    try:
        ...
        """, (
            ...
            data.get("campaign_text", ""),  # ✅ Текст ролика сохраняется
